"""
build_features.py – Feature Engineering: Transacciones limpias → RFM cal/holdout
==================================================================================
Responsabilidad única: transformar el historial de transacciones en la
matriz matemática que necesitan los modelos BG/NBD y Gamma-Gamma,
aplicando un corte temporal estricto (calibración / holdout) para
garantizar que el entrenamiento nunca "vea" el futuro.

Lógica del corte temporal
--------------------------
  · Periodo de CALIBRACIÓN  → desde la primera transacción hasta CUTOFF_DATE
    (aquí se calculan frequency, recency, T y monetary_value).
  · Periodo de HOLDOUT      → desde CUTOFF_DATE hasta OBS_END_DATE
    (aquí se mide el gasto real para validar las predicciones).

  El dataset cubre 2016-09-15 → 2018-08-29 (~23 meses).
  Por defecto dejamos los últimos 6 meses como holdout:
    CUTOFF_DATE   = 2018-02-28  (fin del periodo de calibración)
    OBS_END_DATE  = 2018-08-29  (fin de la observación / holdout)

Columnas de salida (rfm_cal_holdout.parquet)
---------------------------------------------
  customer_unique_id              – ID estable del cliente
  frequency_cal                  – nº de compras repetidas en calibración
  recency_cal                    – tiempo entre 1ª y última compra en cal. (semanas)
  T_cal                          – antigüedad del cliente al CUTOFF (semanas)
  monetary_value_cal             – gasto medio por transacción repetida en cal.
  frequency_holdout              – nº de compras en el periodo holdout
  duration_holdout               – duración del periodo holdout (semanas)
